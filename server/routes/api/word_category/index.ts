...
const wordCategoryRoutes = (server: Application, isDev: boolean) => {
  server.get(
    ...
    async (request: Request, response: Response) => {
      ...
      }/${WP_V2_ENDPOINT}/word_category?slug=${slug}&per_page=${ITEMS_PER_REQUEST}`;
  ...
  server.get(
    '/word_category_words/:slug',
    async (request: Request, response: Response) => {
      const { slug } = request.params;
      const url = `${
        isDev ? DEV_FETCH_URL : PROD_FETCH_URL
      }/${BETTER_REST_ENDPOINT}/word_category/${slug}?per_page=${ITEMS_PER_REQUEST}`;
