...
import { IPostAPIState, postTransform } from '../../../utils/transformers/post.transformer';

const postRoutes = (server: Application, isDev: boolean) => {
  server.get('/post/:slug', async (request: Request, response: Response) => {
    const { slug } = request.params;
    const url = `${isDev ? DEV_FETCH_URL : PROD_FETCH_URL}/${BETTER_REST_ENDPOINT}/post/${slug}`;
