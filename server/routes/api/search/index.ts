...
const searchRoutes = (server: Application, isDev: boolean) => {
  server.get('/search?*', async (request: Request, response: Response) => {
    ...
    }/${WP_V2_ENDPOINT}/search?subtype=${type}&search=${term}&per_page=${ITEMS_PER_REQUEST}`;

    const handleError = error => {
      response
        .status(
          error.res ? error.res.status : error.status ? error.status : 404
        )
        .send(error);
    };
    ...
    try {
      ...
      if (type === 'word') {
        ...
        handleError({
          status: 401,
          message: 'Please only search for "Words", or "Posts"',
        });
      }
