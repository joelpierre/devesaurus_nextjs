...
import menuTransform, { IMenuAPIState } from '../../../utils/transformers/menu.transformer';
...
const menuRoutes = (server: Application, isDev: boolean) => {
  server.get('/menus/:menu', async (request: Request, response: Response) => {
    ...
    const url = `${isDev ? DEV_FETCH_URL : PROD_FETCH_URL}/${BETTER_REST_ENDPOINT}/menus/${menu}`;

    try {
      const fetchedMenu: IMenuAPIState[] = await fetch(url).then(res => res.json());
